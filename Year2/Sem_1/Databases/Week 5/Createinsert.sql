...
insert into animalsale values(2,7,'12 MAR 2014');
-- Commit included to persist the data
commit;

--3.	Peter is having a sale. Write the SQL to update the reduce the price of cats to half price.
update species set speciesprice = (speciesprice/2) where speciesname='Cat';

--4.Ms.  Byrne who bought Sid on 04 Sep 2014 is unhappy with the sale and has returned the snake to the pet store.  What changes need to be made to the data to reflect this?
--(Just use the customer id and the animal id)

delete from animalsale where custid=2 and animalid=7;

--5.	Alter the table animal to include a check that the price of a pet is always less than 100.
alter table species 
add constraint check_price check (speciesprice <100);
