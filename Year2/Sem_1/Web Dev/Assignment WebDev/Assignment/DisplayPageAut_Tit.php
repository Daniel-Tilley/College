		...
		<div align = "center">	
			<?php
				...
				{ 
					...
					$title = $_SESSION['Tit_search1'];
					$author = $_SESSION['Aut_search1'];
					
					//calculate the total number of rows in table 
					$total_quer ='
						SELECT * FROM Books 
						WHERE (BookTitle LIKE "%'.$title.'%") AND (Author LIKE "%'.$author.'%");';
					...
					$quer = '
						SELECT ISBN, BookTitle, Author, Edition, BookYear, Reserved FROM Books 
						WHERE (BookTitle LIKE "%'.$title.'%") AND (Author LIKE "%'.$author.'%") LIMIT '.$start_from.',5;';
					...
					if($result->num_rows > 0)
					{
						...
						for ($i = 1; $i<=$max_page; $i++) 
						{
							...
							echo "
								...
								href='DisplayPageAut_Tit.php?page=".$i."'>Page ".$i."</a> 
							</span>
							"; 
						}//end for
					...
					{
						echo "<p class = 'error'>No books can be found that match the Author and Title</p>";
