...
void ConfigPins()
{
	...
	GPIO1DIR &= ~(ROW_1 | ROW_2 | ROW_3 | ROW_4);
...
{	
	...
	while(1) 
	{
		...
		else 
		{
			if(key=='#')
			{
				DisplayMemory[0]=0x76 << 5;
				DisplayMemory[1]=0x6d << 5;
				DisplayMemory[2]=0x77 << 5;
				DisplayMemory[3]=0x76 << 5;
			}
			
			if(key=='*')
			{
				DisplayMemory[0]=0x50 << 5;
				DisplayMemory[1]=0x77 << 5;
				DisplayMemory[2]=0x78 << 5;
				DisplayMemory[3]=0x6d << 5;
			}
		}
	}    
}
